   ...
   public Rectangle zonaSalida;
    ...
    private ArrayList <ItemsMapaTiled> itemsMapa= new ArrayList<>();
     private ArrayList <Enemigos> enemigosMapa= new ArrayList<>();
       private ArrayList <Enemigos> jefesMapa= new ArrayList<>();
...
for(int i=0; i< coleccionEnemigos.size(); i++){
  ...
  JSONArray coleccionJefes= getArrayJSON(globalJSON.get("jefes").toString());

for(int i=0; i< coleccionJefes.size(); i++){
    
    JSONObject datosJefes = getObjetoJSON(coleccionJefes.get(i).toString());
    
    
    
    int idJefe= getIntJson(datosJefes,"id");
      
        int xJefe= getIntJson(datosJefes ,"x");
          int yJefe= getIntJson(datosJefes ,"y");
          
          
          Point posicJefe= new Point(xJefe, yJefe);
          
          Enemigos Jefes= EnemigosDelJuego.obtenEnemigos(idJefe);
         
          
          
          Jefes.setPosicion(posicJefe.x, posicJefe.y);
...
infoSalida();
 ...
 //  actualizaZonaSalida();
    ...
    private void ataqueEnemigo(){ //ataque del enemigo hacia el jugador
   ...
   private void actualizarAtaquesDeEnemigos(){ //persecucion del enemigo  al pasar por su rango de vision
      ...
      // istrue=false;
                ...
                recc=new Rectangle();
                     ...
                     if(enemigosMapa.isEmpty() && jefesMapa.isEmpty()){
...
if(ObjetosEstaticos.persona.getAlcancePersona().get(0).intersects(i.getAreaEnemigo3b())){
  ...
  if( 
    ...
    i.actualizarr(enemigosMapa);  
       ...
       System.out.println("aqui3");
         ...
         if(!jefesMapa.isEmpty()){
        
         for(Enemigos i : jefesMapa){
            


                      i.actualizarr(jefesMapa);  
    ...
    public void dibujar(Graphics g){  
        ...
        //dibujado de las capas de sprites
        ...
        //dibujado de los items en el suelo del mapa
 ...
 if(puntoX < 0 - Constantes.LADO_SPRITE ||   
        ...
        for(int i=0; i < jefesMapa.size();i++){
            Enemigos jefes= jefesMapa.get(i);
            
            
            int puntoX= (int)jefes.getPosicX() - 
                   ObjetosEstaticos.persona.obtenIntPosicX() + Constantes.MARGEN_X;
            
            int puntoY= (int)jefes.getPosicY() - 
                    ObjetosEstaticos.persona.obtenIntPosicY() + Constantes.MARGEN_Y;
        ...
        jefes.dibujarr(g, puntoX, puntoY);
    ...
    private void actualizaPersonajeAtaques(){  //distintos tipos de ataques de armas de parte del jugador
        
        
        
        
        //ARMAS QUE HIEREN A LOS ENEMIGOS
        ...
        if(enemigosMapa.isEmpty() || ObjetosEstaticos.persona.getAlcanceAtaquePersonaje().isEmpty()   ||
         ...
         if(enemigosMapa.isEmpty() || ObjetosEstaticos.persona.getAlcanceAtaquePersonaje().isEmpty()   ||
            ...
            return;
             ...
             //ATAQUE PENETRANTE Y NO PENETRANTE A LOS ENEMIGOS
        ...
        if (ObjetosEstaticos.teclado.ataque){
            ...
            //definimos el caso de que el ataque atraviese al enemigo afectando a otros o no
       ...
       enemigosAlcanzados.add(enemigo); //agrega al o los enemigos alcanzados por el impacto //ALCANZA A LOS CERCANOS
          ...
          for(Enemigos jefes: jefesMapa){
              
              
              
              
                       
       if(ObjetosEstaticos.persona.getAlcanceAtaquePersonaje().get(0).intersects(jefes.getAreaEnemigo2()) ){
                    
         
                  
       enemigosAlcanzados.add(jefes); //agrega al o los enemigos alcanzados por el impacto //ALCANZA A LOS CERCANOS
        ...
        //se remueve al enemigo del arraylist usando iterator para que no tire un error
        
       Iterator <Enemigos> iterador = enemigosMapa.iterator();
       
       
              Iterator <Enemigos> iterador2 = jefesMapa.iterator();
        ...
        while(iterador.hasNext()){
           ...
           while(iterador2.hasNext()){
            ...
            Enemigos jefe = iterador2.next();
            
            
            if(jefe.getVidaActual() <= 0 ){
       ...
       /* 
        ...
        if (ObjetosEstaticos.teclado.ataque){
          ...
          ArrayList<Enemigos> jefesAlcanzados = new ArrayList<>();  
          
         if(ObjetosEstaticos.persona.getEquipoPersonaje().getPistola().esPenetrantee()) { // si el arma es penetrante (alcanza a varios enemigos)
             
             
              for(Enemigos jefe : jefesMapa){
              
              
           
              
       if(ObjetosEstaticos.persona.getAlcanceAtaquePersonaje().get(0).intersects(jefe.getAreaEnemigo2()) ){
                    
         
                  
       jefesAlcanzados.add(jefe); //agrega al o los enemigos alcanzados por el impacto //ALCANZA A LOS CERCANOS
                    
          
          //      System.out.println("daño a jefe");   
       } 
             ...
             ObjetosEstaticos.persona.getEquipoPersonaje().getPistola().atacarJef(jefesAlcanzados);            
                     
                         
             ObjetosEstaticos.persona.getEquipoPersonaje().getEspada().atacarJef(jefesAlcanzados); 
